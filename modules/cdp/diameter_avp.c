/**
 * $Id: diameter_avp.c 1144 2011-08-30 11:20:35Z vingarzan $
 *
 ...
 *  \author Umut Emin umut dot emin -at- fokus dot fraunhofer dot de
 *
 *  15-09-2006 Changed arg 'unsigned int length' to 'size_t length'
 * 	at function AAACreateAVP , line 82 (Umut Emin aka jsbach)
 */
	...
	AVPDataStatus data_status)
{
	...
	if( data==0 || length==0) {
		LM_ERR("NULL value received for"
	...
	AAA_AVP *position)
{
	...
	if (!position) {
		...
		if (!avp_t) {
			LM_ERR("the \"position\" avp is not in"
				"\"msg\" message!!\n");
			return AAA_ERR_PARAMETER;
		}
...
AAA_AVP_LIST AAAUngroupAVPS(str buf)
{
	...
	while (ptr < buf.s+buf.len) {
		...
		if ( buf.s+buf.len<ptr+avp_data_len) {
			LM_ERR("source buffer to short!! Cannot read a whole data for AVP!\n");
			goto error;
		}
