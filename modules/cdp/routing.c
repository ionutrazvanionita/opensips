/**
 * $Id: routing.c 920 2010-03-05 19:07:50Z vingarzan $
 *
...
peer* get_routing_peer(AAAMessage *m)
{
	...
	if (destination_realm.len){
		...
		if (rr) {
			...
			if (p) return p;
			else LM_ERR("No connected Route peer found for Realm <%.*s>. Trying DefaultRoutes next...\n",
	...
	if (!p){
		LM_ERR("No connected DefaultRoute peer found for app_id %d and vendor id %d.\n",
				app_id,vendor_id);
	}
	return p;
}
