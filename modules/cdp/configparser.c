/**
 * $Id: configparser.c 1078 2011-04-20 09:33:35Z vingarzan $
 *
 ...
 * @returns 1 always
 */
static inline int parser_init(void)
{
 ...
 * Destroys the parser
 */
static inline void parser_destroy(void)
{
	xmlCleanupParser();
}
...
static inline void quote_trim_dup(str *dest, char *src)
{
	...
	if (!dest->s) {
		LOG_NO_MEM("shm",(long int)dest->len);
		dest->len=0;
		return;
	}
...
xmlDocPtr parse_dp_config_file(char* filename)
{
	...
	if (!doc){
		LM_ERR("ERR:parse_dp_config_file():  This is not a valid XML file <%s>\n",
			filename);
		goto error;
	}
...
xmlDocPtr parse_dp_config_str(str config_str)
{
	...
	if (!doc){
		LM_ERR("ERR:parse_dp_config_file():  This is not a valid XML string <%.*s>\n",
...
dp_config* parse_dp_config(xmlDocPtr doc)
{
	...
	if (!root){
		LM_ERR("ERR:parse_dp_config():  Empty XML \n");
		goto error;
	}
	...
	if (strncasecmp((char*)root->name,"DiameterPeer",k)!=0){
		LM_ERR("ERR:parse_dp_config(): XML Root is not <DiameterPeer>\n");
		goto error;
	}
