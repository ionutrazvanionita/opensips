/**
 * $Id: peerstatemachine.c 1078 2011-04-20 09:33:35Z vingarzan $
 *
			...
			switch(event){
				case I_Rcv_Conn_Ack:
					...
					if (p->r_cer){
						...
						if (Elect(p,p->r_cer)){
							// won the election = > I_Disc(), R_Send_CEA()
							LM_INFO("Wait_Conn_Ack_Elect Win Elect \n");
							sm_process(p,Win_Election,p->r_cer,1,sock);
							p->r_cer = 0;
						} else {
							// lost the election => wait for I_Recv_CEA, then R_Disc()
							LM_INFO("Wait_Conn_Ack_Elect Lose Elect \n");
							AAAFreeMessage(&p->r_cer);
						}
					} else {
						LM_ERR("Wait_Conn_Ack_Elect, I_Rcv_Conn_Ack, No R-CER ! \n");
						p->state = Wait_I_CEA;
					}
					break;
				case I_Rcv_Conn_NAck:
					...
					if (p->r_cer){
						...
						LM_ERR("Wait_Conn_Ack_Elect, I_Rcv_Conn_NAck No R-CER ! \n");
					}
					break;
				case R_Peer_Disc:
					R_Disc(p);
			...
			switch(event){
				...
				case I_Peer_Disc:
					...
					if (p->r_cer){
						...
						LM_ERR("Wait_Returns, I_Peer_Disc No R-CER ! \n");
					}
					break;
				case I_Rcv_CEA:
...
void save_peer_applications(peer *p,AAAMessage *msg)
{
	...
	if (!p->applications){
		LM_ERR("Error allocating %ld bytes! No applications saved...\n",
...
void Rcv_Process(peer *p, AAAMessage *msg)
{
	...
	//if (msg) LM_ERR("DBG:Rcv_Process(): task added to queue command %d, flags %#1x endtoend %u hopbyhop %u\n",msg->commandCode,msg->flags,msg->endtoendId,msg->hopbyhopId);
