...
void cb_remove(cdp_cb_t *cb)
{
	...
	int worker_execute(void* ptr)
	{
		...
		LM_DBG("Executing task for cdp: Message [%d] from Peer [%.*s]\n",
	...
	int put_task(peer *p,AAAMessage *msg)
	{
		...
		if (!t){
			LM_ERR("Error allocating %ld bytes of shm!\n",(long int)sizeof(task_t));
			return 0;
		}
		t->p = p;
		t->msg = msg;
...
void worker_process(int id)
{
	...
	srandom(time(0)*(WORKER_PROCESS_ID_START + id));

	LM_INFO("[%d]Worker process started...\n",id);

	/* add worker process pid in list */
	dp_add_pid(getpid());
	...
	LM_GEN1(memlog, "Worker[%d] Memory status (pkg):\n",id);
	//pkg_status();
	dp_del_pid(getpid());
