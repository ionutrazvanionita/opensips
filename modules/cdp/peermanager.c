/**
 * $Id: peermanager.c 937 2010-04-28 11:37:18Z vingarzan $
 *
...
int peer_manager_init(dp_config *config)
{
	int i;
	peer *p;
	LM_DBG("DBG:peer_manager_init(): Peer Manager initialization...\n");
 ...
 */
void peer_manager_destroy(void)
{
	peer *foo,*bar;
...
/*	lock_get(msg_id_lock);	*/
	...
	LM_DBG("DBG:peer_manager_init(): ...Peer Manager destroyed\n");
}
...
void log_peer_list(int level)
{
	...
	LM_GEN1(level,"--- Peer List: ---\n");
	for(p = peer_list->head;p;p = p->next){
		LM_GEN1(level,ANSI_GREEN" S["ANSI_YELLOW"%s"ANSI_GREEN"] "ANSI_BLUE"%.*s:%d"ANSI_GREEN" D["ANSI_RED"%c"ANSI_GREEN"]\n",dp_states[p->state],p->fqdn.len,p->fqdn.s,p->port,p->is_dynamic?'X':' ');
		for(i=0;i<p->applications_cnt;i++)
			LM_GEN1(level,ANSI_YELLOW"\t [%d,%d]"ANSI_GREEN"\n",p->applications[i].id,p->applications[i].vendor);
	}
	LM_GEN1(level,"------------------\n");
}
 ...
 */
inline AAAMsgIdentifier next_hopbyhop(void)
{
	AAAMsgIdentifier x;
	lock_get(msg_id_lock);
 ...
 * @returns the new identifier to be used in messages
 */
inline AAAMsgIdentifier next_endtoend(void)
{
	AAAMsgIdentifier x;
	lock_get(msg_id_lock);
