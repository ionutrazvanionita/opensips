/**
 * $Id: timer.c 935 2010-04-28 10:32:33Z vingarzan $
 *
...
int add_timer(int expires_in,int one_time,callback_f cb,void *ptr)
{
	timer_cb_t *n;
	if (expires_in==0){
		LM_ERR("Minimum expiration time is 1 second!\n");
		return 0;
	}
...
void timer_process(int returns)
{
	...
	if (!returns) {
		LM_GEN1(memlog, "Timer Memory status (pkg):\n");
		//pkg_status();
		dp_del_pid(getpid());
