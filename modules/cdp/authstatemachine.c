/**
 * $Id: authstatemachine.c 959 2010-07-20 08:43:13Z vingarzan $
 *
...
inline void auth_client_stateless_sm_process(cdp_session_t* s, int event, AAAMessage *msg)
{
	...
	switch(x->state){
		case AUTH_ST_IDLE:
			switch(event){
				case AUTH_EV_SEND_REQ:
					...
					break;
				default:
					LM_ERR("ERR:auth_client_stateless_sm_process(): Received invalid event %d while in state %s!\n",
						event,auth_states[x->state]);
			}
			break;
		...
		default:
			LM_ERR("ERR:auth_client_stateless_sm_process(): Received event %d while in invalid state %d!\n",
				event,x->state);
	}
...
/*
	...
	switch(x->state){
		case AUTH_ST_IDLE:
			switch(event){
				default:
					LM_ERR("ERR:auth_server_stateless_sm_process(): Received invalid event %d while in state %s!\n",
						event,auth_state[x->state]);
			}
			break;
		default:
			LM_ERR("ERR:auth_server_stateless_sm_process(): Received event %d while in invalid state %d!\n",
				event,x->state);
	}
*/
	if (s) AAASessionsUnlock(s->hash);
...
int dup_routing_avps(AAAMessage* src, AAAMessage *dest){
	...
	if(avp && avp->data.s && avp->data.len) {
		LM_DBG("DBG:dup_routing_avps: Origin Host AVP present, duplicating %.*s\n",
		...
		if (!avp) {
			LM_ERR("ERR:dup_routing_avps: Failed creating Destination Host avp\n");
			goto error;
		}
		if (AAAAddAVPToMessage(dest,avp,dest->avpList.tail)!=AAA_ERR_SUCCESS) {
			LM_ERR("ERR:dup_routing_avps: Failed adding Destination Host avp to message\n");
			AAAFreeAVP(&avp);
			goto error;
		}
	}
	...
	if(avp && avp->data.s && avp->data.len) {
		LM_DBG("DBG:dup_routing_avps: Origin Realm AVP present, duplicating %.*s\n",
				avp->data.len, avp->data.s);
		dest_realm = avp->data;
...
void Send_STR(cdp_session_t* s, AAAMessage* msg)
{
	...
	if (!str) {
		LM_ERR("ERR:Send_STR(): error creating STR!\n");
		return;
	}
	if(!dup_routing_avps(msg, str)){
		LM_ERR("ERR:Send_STR(): error duplicating routing AVPs!\n");
		AAAFreeMessage(&str);
		return;
	}
	if(s->vendor_id!=0 && !add_vendor_specific_application_id_group(str,s->vendor_id,s->application_id)){
		LM_ERR("ERR:Send_STR(): error adding Vendor-Id-Specific-Application-Id Group!\n");
		AAAFreeMessage(&str);
		return;
	}
...
void Send_ASR(cdp_session_t* s, AAAMessage* msg)
{
	...
	if (!asr) {
		LM_ERR("ERR:Send_ASR(): error creating ASR!\n");
		return;
	}
