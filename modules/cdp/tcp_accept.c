/**
 * $Id: tcp_accept.c 920 2010-03-05 19:07:50Z vingarzan $
 *
...
int create_socket(int listen_port,str bind_to,unsigned int *sock)
{
	...
	if (bind_to.len){
		...
		if (error!=0){
			LM_WARN("Error opening %.*s port %d while doing gethostbyname >%s\n",
		...
		if (error!=0){
			LM_WARN("Error opening ANY port %d while doing gethostbyname >%s\n",
				listen_port,gai_strerror(error));
			goto error;
		}
	}

	LM_DBG("create socket and bind for IPv4...\n");
	...
	for(ainfo = res;ainfo;ainfo = ainfo->ai_next)
	{
		if (getnameinfo(ainfo->ai_addr,ainfo->ai_addrlen,
			host,256,serv,256,NI_NUMERICHOST|NI_NUMERICSERV)==0){
				LM_WARN("Trying to open/bind/listen on %s port %s\n",
					host,serv);
		}

		if ((server_sock = socket(ainfo->ai_family, ainfo->ai_socktype, ainfo->ai_protocol)) == -1) {
			LM_ERR("error creating server socket on %s port %s >"
		...
		*sock = server_sock;
		...
		LM_WARN("Successful socket open/bind/listen on %s port %s\n",
					host,serv);
	}
