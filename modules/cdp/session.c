	...
	if (x){
		...
		switch(x->type){
			case UNKNOWN_SESSION:
				if (x->u.generic_data){
					LM_ERR("The session->u.generic_data should be freed and reset before dropping the session!"
...
void cdp_sessions_log(int level)
{
	...
	LM_GEN1(level,"------- CDP Sessions ----------------\n");
	for(hash=0;hash<sessions_hash_size;hash++){
		AAASessionsLock(hash);
		for(x = sessions[hash].head;x;x=x->next) {
			LM_GEN1(level," %3u. [%.*s] AppId [%d] Type [%d]\n",
					hash,
					x->id.len,x->id.s,
					x->application_id,
					x->type);
			switch (x->type){
				case AUTH_CLIENT_STATEFULL:
				case AUTH_SERVER_STATEFULL:
					LM_GEN1(level,"\tAuth State [%d] Timeout [%d] Lifetime [%d] Grace [%d] Generic [%p]\n",
							x->u.auth.state,
