/**
 * $Id: diameter_peer.c 936 2010-04-28 11:28:30Z vingarzan $
 *
 ...
 */
int diameter_peer_init_real(void)
{
	pid_list_t *i,*j;
 ...
 */
int diameter_receiver_init(void)
{
	int k=0;
	peer *p;
 ...
 */
void diameter_peer_destroy(void)
{
	int pid,status;
	handler *h;
	...
	LM_INFO("Terminating all childs...\n");
	while(pid_list->tail){
		pid = dp_last_pid();
		if (pid<=0||pid==getpid()){
			dp_del_pid(pid);
