/**
 * $Id: receiver.c 1113 2011-06-08 09:09:48Z aon $
 *
...
static void log_serviced_peers(int level)
{
	...
	/* FIXME */
	/* FIXME */
	LM_GEN1(level,"--- Receiver ["ANSI_BLUE"%s"ANSI_GREEN"] Serviced Peers: ---\n",
			pt[process_no].desc
			);
	for(sp=serviced_peers;sp;sp=sp->next){
		LM_GEN1(level,ANSI_GREEN" Peer: ["ANSI_YELLOW"%.*s"ANSI_GREEN"]  TCP Socket: ["ANSI_YELLOW"%d"ANSI_GREEN"] Recv.State: ["ANSI_YELLOW"%d"ANSI_GREEN"]\n",
...
{
	serviced_peer_t *sp;
	LM_INFO("Adding serviced_peer_t to receiver for peer [%.*s]\n",
			p?p->fqdn.len:0,
			p?p->fqdn.s:0);
...
/* FIXME remove this define and we have errors */
struct cmsghdr* cmsg;
	union{
		struct cmsghdr cm;
...
again:
	...
	if (ret!=sizeof(peer *)){
		LM_WARN( "different number of bytes received than expected (%d from %ld)"
	...
	if ((cmsg!=0) && (cmsg->cmsg_len==CMSG_LEN(sizeof(new_fd)))){
		if (cmsg->cmsg_type!= SCM_RIGHTS){
			LM_ERR( "ERROR: receive_fd: msg control type != SCM_RIGHTS\n");
			goto error;
		}
		if (cmsg->cmsg_level!= SOL_SOCKET){
			LM_ERR( "ERROR: receive_fd: msg level != SOL_SOCKET\n");
			goto error;
		}
		*fd=*((int*) CMSG_DATA(cmsg));
	}else{
		if(!cmsg)
			LM_ERR( "ERROR: receive_fd: no descriptor passed, empty control message");
		else
			LM_ERR( "ERROR: receive_fd: no descriptor passed, cmsg=%p,"
	...
	if (msg.msg_accrightslen==sizeof(int)){
		*fd=new_fd;
		...
		LM_ERR( "ERROR: receive_fd: no descriptor passed,"
...
void receiver_process_w(int id)
{
	int k;
	peer* p;

	srandom(time(0) * (RECEIVER_PROCESS_ID_START+id));
	dp_add_pid(getpid());
	...
	if (id==0) {
		...
		/* process with id 0 is the unknown peer process(if we accept unknwo peers);
		 * processes starting from k=1 to k=config->*/
		for (k=1, p=peer_list->head; k != id; k++, p=p->next);
...
{
	LM_INFO("[%.*s] Receiver process doing init on new process...\n",
	...
	if (!*shutdownx){
		LM_INFO("[%.*s]... Receiver process cleaning-up - should not happen unless shuting down!\n",
				p?p->fqdn.len:0,p?p->fqdn.s:0);
	...
	LM_GEN1(memlog, "Receiver[%.*s] Memory status (pkg):\n",
			p?p->fqdn.len:0,p?p->fqdn.s:0);
	//pkg_status();
	...
	switch (sp->state){
		...
		case Receiver_Header:
			...
			if (sp->buf_len==DIAMETER_HEADER_LEN){
				...
				LM_DBG("[%.*s] Recv Version %d Length %d\n",
						sp->p?sp->p->fqdn.len:0,
						sp->p?sp->p->fqdn.s:0,
						(unsigned char)(sp->buf[0]),
						sp->length);
				...
				if (!sp->msg) {
					LOG_NO_MEM("shm",(long int)sp->length);
					goto error_and_reset;
				}
...
//	if (shutdownx) return -1;
	...
	//		LM_CRIT("ERROR:select_recv(): HERE\n");
			...
			if (n==-1){
				...
				if (n){
					...
					if (FD_ISSET(fd_exchange_pipe_local,&rfds)){
						/* fd exchange */
						LM_DBG("There is something on the fd exchange pipe\n");
						p = 0;
						fd = -1;
						if (!receive_fd(fd_exchange_pipe_local,&fd,&p)){
							LM_ERR("Error reading from fd exchange pipe\n");
						}else{
							LM_DBG("fd exchange pipe says fd [%d] for peer %p:[%.*s]\n",fd,
									p,
									p?p->fqdn.len:0,
									p?p->fqdn.s:0);
							if (p){
					...
					for(sp=serviced_peers;sp;){
						if (sp->tcp_socket>=0 && FD_ISSET(sp->tcp_socket,&efds)) {
							LM_INFO("[%.*s] Peer socket [%d] found on the exception list... dropping\n",
									sp->p?sp->p->fqdn.len:0,
									sp->p?sp->p->fqdn.s:0,
									sp->tcp_socket);
							goto drop_peer;
						...
						if (sp->send_pipe_fd>=0 && FD_ISSET(sp->send_pipe_fd,&rfds)) {
							/* send */
							LM_DBG("There is something on the send pipe\n");
							cnt = read(sp->send_pipe_fd,&msg,sizeof(AAAMessage *));
							if (cnt==0){
								//This is very stupid and might not work well - droped messages... to be fixed
								LM_INFO("ReOpening pipe for read. This should not happen...\n");
								close(sp->send_pipe_fd);
								sp->send_pipe_fd = open(sp->send_pipe_name.s, O_RDONLY | O_NDELAY);
								goto receive;
							}
							if (cnt<sizeof(AAAMessage *)){
								if (cnt<0) LM_ERR("Error reading from send pipe\n");
								goto receive;
							}
							LM_DBG("Send pipe says [%p] %d\n",msg,cnt);
							if (sp->tcp_socket<0){
								LM_ERR("got a signal to send something, but the connection was not opened");
							} else {
								while( (cnt=write(sp->tcp_socket,msg->buf.s,msg->buf.len))==-1 ) {
									if (errno==EINTR)
										continue;
									LM_ERR("[%.*s] write on socket [%d] returned error> %s... dropping\n",
											sp->p?sp->p->fqdn.len:0,
											sp->p?sp->p->fqdn.s:0,
											sp->tcp_socket,
											strerror(errno));
								...
								if (cnt!=msg->buf.len){
									LM_ERR("[%.*s] write on socket [%d] only wrote %d/%d bytes... dropping\n",
											sp->p?sp->p->fqdn.len:0,
											sp->p?sp->p->fqdn.s:0,
											sp->tcp_socket,
											cnt,
	...
	receive:
						...
						if (sp->tcp_socket>=0 && FD_ISSET(sp->tcp_socket,&rfds)) {
							...
							cnt = do_receive(sp);
							if (cnt<=0) {
								LM_INFO("[%.*s] read on socket [%d] returned %d > %s... dropping\n",
										sp->p?sp->p->fqdn.len:0,
										sp->p?sp->p->fqdn.s:0,
										sp->tcp_socket,
										cnt,
	...
	for(ainfo = res;ainfo;ainfo = ainfo->ai_next)
	{
		if (getnameinfo(ainfo->ai_addr,ainfo->ai_addrlen,
			host,256,serv,256,NI_NUMERICHOST|NI_NUMERICSERV)==0){
				LM_WARN("Trying to connect to %s port %s\n",
					host,serv);
		}
			...
			if (res<0){
				if (errno==EINPROGRESS){
					  ...
					  if (select(sock+1, NULL, &myset, NULL, &tv) > 0) {
						  ...
						  if (valopt) {
					    	  LM_WARN("Error opening connection to to %s port %s >%s\n",host,serv,strerror(valopt));
					    	  close(sock);
					    	  continue;
					      }
					  }else{
				    	  LM_WARN("Timeout or error opening connection to to %s port %s >%s\n",host,serv,strerror(errno));
				    	  close(sock);
				    	  continue;
					  }
				}
...
int peer_send_msg(peer *p,AAAMessage *msg)
{
	...
	if (n!=sizeof(AAAMessage *)) {
		LM_ERR("Peer %.*s error on pipe write > only %d bytes written\n",p->fqdn.len,p->fqdn.s,n);
		close(fd);
		return 0;
	}
	close(fd);
	...
	if (!sp->p){
		switch (msg->commandCode){
			case Code_CE:
				if (is_req(msg)){
					...
					if (!sp->p) {
						LM_ERR("Received CER from unknown peer (accept unknown=%d) -ignored\n",
		...
		switch (sp->p->state){
			...
			case R_Open:
				...
				break;
			default:
				LM_ERR("[%.*s] Received msg while peer in state %d -ignored\n",
						sp->p->fqdn.len,
						sp->p->fqdn.s,
						sp->p->state);
				AAAFreeMessage(&msg);
